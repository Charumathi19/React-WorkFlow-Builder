...
import { createCase, runCase, getDomains, getKeywords } from '../lib/api';
import type { Domain, Keyword, NewCasePayload } from '../types';
...
export default function NewCase() {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const { case_id } = await createCase(payload);
            await runCase(case_id);
            navigate(`/ams/cases/${case_id}`);
        } catch (err: unknown) {
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                <div
                                    style={{
                                        ...
                                        background: i < step ? '#22c55e' : i === step ? '#e8530a' : '#1e1e1e',
                ...
                <div className="ams-card" style={{ maxWidth: 680 }}>
                                ...
                                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginTop: 4 }}>
                                    {LANGUAGES.map((lang) => (
                                        <button
                                            ...
                                            style={{
                                                background: languages.includes(lang) ? 'rgba(232,83,10,0.15)' : '#1a1a1a',
                                                color: languages.includes(lang) ? '#e8530a' : '#666',
                                                border: `1px solid ${languages.includes(lang) ? 'rgba(232,83,10,0.4)' : '#2a2a2a'}`,
                                            }}
                    ...
                    {step === 1 && (
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                            <div className="ams-form-group">
                                ...
                                <select className="ams-select" value={details.entity_type}
                                    onChange={(e) => setDetails((d) => ({ ...d, entity_type: e.target.value as '' | 'person' | 'organization' }))}>
                            ...
                            <div className="ams-form-group">
                                ...
                                <select className="ams-select" value={details.gender}
                                    onChange={(e) => setDetails((d) => ({ ...d, gender: e.target.value as '' | 'male' | 'female' }))}>
                                    <option value="">— Select —</option>
                            ...
                            <div className="ams-form-group">
                                ...
                                <input type="date" className="ams-input" value={details.date_of_birth}
                                    onChange={(e) => setDetails((d) => ({ ...d, date_of_birth: e.target.value }))} />
                            </div>
                            <div className="ams-form-group">
                                <label className="ams-form-label">Nationality (ISO)</label>
                                <input className="ams-input" placeholder="e.g. IL, MY" value={details.nationality}
                                    onChange={(e) => setDetails((d) => ({ ...d, nationality: e.target.value.toUpperCase() }))} maxLength={2} />
                            </div>
                            <div className="ams-form-group">
                                <label className="ams-form-label">Place of Birth</label>
                                <input className="ams-input" placeholder="City, Country" value={details.place_of_birth}
                                    onChange={(e) => setDetails((d) => ({ ...d, place_of_birth: e.target.value }))} />
                            </div>
                            <div className="ams-form-group">
                                <label className="ams-form-label">ID Number</label>
                                <input className="ams-input" placeholder="Passport / National ID" value={details.id_number}
                                    onChange={(e) => setDetails((d) => ({ ...d, id_number: e.target.value }))} />
                            </div>
                            <div className="ams-form-group" style={{ gridColumn: '1/-1' }}>
                                <label className="ams-form-label">Address</label>
                                <input className="ams-input" placeholder="Full address" value={details.address}
                                    onChange={(e) => setDetails((d) => ({ ...d, address: e.target.value }))} />
                            </div>
                            <div className="ams-form-group" style={{ gridColumn: '1/-1' }}>
                                <label className="ams-form-label">Aliases</label>
                                <div style={{ display: 'flex', gap: 8 }}>
                                    <input className="ams-input" placeholder="Add alias…" value={aliasInput}
                                ...
                                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 8 }}>
                                    {details.aliases.map((a, i) => (
                                        <span key={i} style={{ background: '#1e1e1e', color: '#aaa', padding: '4px 10px', borderRadius: 6, fontSize: 12, display: 'flex', alignItems: 'center', gap: 5 }}>
                                            {a}
                                            <button onClick={() => removeAlias(i)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#666', padding: 0 }}><X size={11} /></button>
                                        </span>
                                    ))}
                                </div>
                            </div>
                                ...
                                <div style={{ maxHeight: 200, overflowY: 'auto', background: '#111', border: '1px solid #2a2a2a', borderRadius: 8, padding: 8 }}>
                                    {domains.filter(d => d.active).map((d) => (
                                        <label key={d.domain} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '5px 6px', cursor: 'pointer', borderRadius: 5, fontSize: 13, color: '#aaa' }}>
                                            ...
                                            {d.domain}
                                            <span style={{ color: '#555', fontSize: 11 }}>tier {d.tier}</span>
                                        </label>
                                    ))}
                                ...
                                <div style={{ maxHeight: 200, overflowY: 'auto', background: '#111', border: '1px solid #2a2a2a', borderRadius: 8, padding: 8 }}>
                                    {keywords.filter(k => k.active).map((k) => (
                                        <label key={k.keyword_en} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '5px 6px', cursor: 'pointer', borderRadius: 5, fontSize: 13, color: '#aaa' }}>
                                            ...
                                            {k.keyword_en}
                                            <span style={{ color: '#555', fontSize: 11 }}>{k.category}</span>
                                        </label>
                                    ))}
                    ...
                    {step === 3 && (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                            <div style={{ display: 'grid', gridTemplateColumns: '140px 1fr', gap: '8px 0', fontSize: 13 }}>
                                <span style={{ color: '#666' }}>Entity</span>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 28, paddingTop: 20, borderTop: '1px solid #1e1e1e' }}>
                        <button
                            ...
                            onClick={() => step === 0 ? navigate('/ams/cases') : setStep(s => s - 1)}
                        >
